...
// Props માં editData ને બદલે product કર્યું જેથી Dashboard સાથે મેચ થાય
type Props = {
  onSuccess: () => void; 
  product?: any; 
};
...
export default function ProductForm({ onSuccess, product }: Props) {
  // લોજિક એ જ રાખ્યું છે, ફક્ત વેરીએબલ નામ બદલ્યું છે
  const isEdit = Boolean(product);
  const currentUsername = localStorage.getItem("username") || "Unknown";

  return (
    <Formik
      enableReinitialize={true} // આ જરૂરી છે જેથી જ્યારે product બદલાય ત્યારે ફોર્મ રિફ્રેશ થાય
      initialValues={{
        name: product?.name || "",
        ...
        update_reason: product?.update_reason || "",
        product_image: null, 
      }}
      ...
      onSubmit={async (values, { resetForm }) => {
        ...
        try {
          if (isEdit) {
            ...
            // product.id નો ઉપયોગ કરીને PUT કોલ
    ...
    >
      {({ setFieldValue, values }) => (
        <Form className="p-1 mb-1 border-0">
          <h5 className="mb-3 text-primary fw-bold p-3">
